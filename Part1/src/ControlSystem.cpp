#include "../includes/ControlSystem.h"
...
ControlSystem::ControlSystem(): 
    playerInputs({Vector2D(0.0f, 0.0f), Vector2D(0.0f, 0.0f)}) {
    // initialization.
}
    ...
    for (int i = 0; i < GameSetting::getInstance()->getPlayerNum(); ++i) {
        ...
        switch (GameSetting::CurrentPlayerControlSchemes[i]) {
            case GameSetting::ControlSchemes::WASD:
                if (e.type == SDL_KEYDOWN)    playerInputs[i] = Vector2D((e.key.keysym.sym==SDLK_d?1.0f:0.0f)-
                                                                           (e.key.keysym.sym==SDLK_a?1.0f:0.0f),
                                                                           (e.key.keysym.sym==SDLK_w?1.0f:0.0f)-
                                                                           (e.key.keysym.sym==SDLK_s?1.0f:0.0f));
                else playerInputs[i] = Vector2D(0.0f, 0.0f);
                
                break;
            case GameSetting::ControlSchemes::IJKL:
                if (e.type == SDL_KEYDOWN)    playerInputs[i] = Vector2D((e.key.keysym.sym==SDLK_l?1.0f:0.0f)-
            ...
            case GameSetting::ControlSchemes::ArrowKeys:
                if (e.type == SDL_KEYDOWN)    playerInputs[i] = Vector2D((e.key.keysym.sym==SDLK_RIGHT?1.0f:0.0f)-
